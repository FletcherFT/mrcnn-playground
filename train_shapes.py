...
# 
# The code of the *Shapes* dataset is included below. It generates images on the fly, so it doesn't require downloading
# any data. And it can generate images of any size, so we pick a small image size to train faster.
...
# ## Training
# Train in two stages: 1. Only the heads. Here we're freezing all the backbone layers and training only the randomly
# initialized layers (i.e. the ones that we didn't use pre-trained weights from MS COCO). To train only the head
# layers, pass `layers='heads'` to the `train()` function.
# 
...
# Save weights
# Typically not needed because callback s save after every epoch
